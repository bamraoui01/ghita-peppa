...
const names=['Peppa','George','Suzy'];
const photo=new Image();photo.src='../ghita.jpg';
function ghita(x,y){ctx.save();ctx.translate(x,y);line([[-18,0],[-18,21]],'#364f72',9);line([[18,0],[18,21]],'#364f72',9);line([[-40,-71],[-66,-92]],'#df4d91',14);line([[40,-71],[66,-92]],'#df4d91',14);ctx.fillStyle='#df4d91';ctx.beginPath();ctx.roundRect(-42,-105,84,116,20);ctx.fill();line([[0,-98],[0,7]],'#ffb9dd',3);ellipse(0,-136,43,49,'#734d3f');if(photo.complete&&photo.naturalWidth){ctx.save();ctx.beginPath();ctx.ellipse(0,-135,38,43,0,0,Math.PI*2);ctx.clip();ctx.drawImage(photo,195,205,240,250,-40,-179,80,86);ctx.restore()}ctx.fillStyle='#7543ad';ctx.beginPath();ctx.roundRect(-45,-186,90,25,10);ctx.fill();ellipse(0,-188,39,21,'#7543ad');ellipse(0,-214,11,11,'#ffcae7');line([[-40,-96],[37,-96]],'#ffcd56',13);line([[28,-96],[31,-66]],'#ffcd56',12);ctx.restore()}
...
function draw(now){
 ...
 ellipse(210,420,420,180,'#d7ebf6');ellipse(800,437,490,190,'#edf7fc');ctx.fillStyle='#f7fcff';ctx.fillRect(0,414,960,126);
 ...
 for(let i=0;i<35;i++){let x=(i*137)%960,y=reduced?(i*83)%480:((i*83)+now*.018)%500;ellipse(x,y,3+(i%3),3+(i%3),'#ffffff')}
 ellipse(451,455,132,31,'#c6e1f2');ellipse(455,451,107,19,'#e3f3fc');ellipse(419,443,43,5,'#ffffff');
 const t=jumping?Math.min((now-jumpAt)/(reduced?280:850),1):0;let lift=jumping?Math.sin(t*Math.PI)*(reduced?25:135):0;
 ellipse(428,440,43-lift*.08,10,'#94bfd780');ghita(427,414-lift);
 ...
 const age=(now-splashAt)/900;if(age>=0&&age<1&&!reduced){for(let i=0;i<12;i++){const a=i/12*Math.PI*2;ellipse(440+Math.cos(a)*age*160,435-Math.sin(Math.abs(a))*age*100+age*age*40,5*(1-age)+2,9*(1-age)+2,'#ffffff')}star(435,300-age*75,24*(1-age)+8,'#ffcf38')}
...
function update(){document.querySelector('#stars').textContent=Array.from({length:6},(_,i)=>i<score?'★':'☆').join(' ');document.querySelector('#stars').setAttribute('aria-label',score+' étoiles sur 6');document.querySelector('#friends').textContent='Amis retrouvés : '+Math.floor(score/2)+' / 3';document.querySelector('#fill').style.width=score/6*100+'%';document.querySelector('#progress').setAttribute('aria-valuenow',score);jumpButton.disabled=false;
 if(score===6){message.textContent='Bravo, Ghita ! Tous les amis sont là !';document.querySelector('#win').hidden=false;jumpButton.textContent='Rejouer ↻';document.querySelector('#hint').textContent='Une autre petite aventure ?'}else{message.textContent=score%2===0?'Coucou '+names[score/2-1]+' ! Encore un saut dans la neige ?':'Youpi ! Une étoile pour toi, Ghita !';jumpButton.textContent='Sauter ! ❄️'}
}
function act(){if(jumping)return;if(score===6){score=0;document.querySelector('#win').hidden=true;update();started=false}if(!started){started=true;message.textContent='Appuie pour sauter dans la neige !';jumpButton.textContent='Sauter ! ❄️';document.querySelector('#hint').textContent='Touche le bouton · ou appuie sur Espace ou ↑';return}jumping=true;jumpAt=performance.now();jumpButton.disabled=true;message.textContent='Hop… dans la neige !'}
